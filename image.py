...
def get_image(soup,brand,sr):
    try:
        img = soup.find('img', attrs={'class': 'tireprev center-block'})
        #if img.get('class') == 'tireprev center-block':
        

        if not img:
            div = soup.find('div', attrs={'class': 'item active'})
            img2 = div.find('img', attrs={'class': 'tireprev center-block resize_fit_center catalog-product-image'})
            img_link = img2.get('src')
        else:
            img_link = img.get('src')

        #elif img.get('class') == 'tireprev center-block resize_fit_center catalog-product-image':
        ...
        urllib.request.urlretrieve(img_link, brand+" "+sr+".jpg")
        time.sleep(3)
...
in1 = brandname.index('Radar Tires') 
in2 = brandURL.index('/radar-tires')
